...
const HELP_MESSAGE = [
  ...
  '`debug on` / `debug off` — паказваць або хаваць RAG-дыягностыку.',
].join('\n');
...
client.on('messageCreate', async (message: Message) => {
  ...
  if (debugCommand) {
    if (debugCommand === 'on') {
      ...
      await message.reply('Debug mode уключаны. Пасля адказу буду паказваць RAG-запыты і знойдзеныя крыніцы.');
      return;
    }
...
function parseDebugCommand(text: string): 'on' | 'off' | undefined {
  ...
  if (/^(\/?debug|\/?dev|д[эе]баг|дэв)\s+(on|ўкл|уключы|так|true)$/iu.test(normalized)) {
    return 'on';
  }

  if (/^(\/?debug|\/?dev|д[эе]баг|дэв)\s+(off|выкл|выключы|не|false)$/iu.test(normalized)) {
    return 'off';
  }
...
function formatRagDebug(result: ChatAgentResponse): string {
  ...
  if (result.trace.queryBreakdown?.length) {
    ...
    for (const item of result.trace.queryBreakdown) {
      lines.push(`- ${item.query} -> retrieved ${item.retrievedCount}, kept ${item.keptCount}`);
    }
  }
  ...
  for (const [index, source] of result.sources.slice(0, 8).entries()) {
    ...
    const relevanceScore = 'relevanceScore' in source && typeof source.relevanceScore === 'number'
    ...
    if (source.matchedQueries?.length) {
      lines.push(`   matched: ${source.matchedQueries.slice(0, 3).join(' | ')}`);
    }
